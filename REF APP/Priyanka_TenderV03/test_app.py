...
def handle_nit_upload():
    """Handle NIT document upload and processing (with full traceback)."""
    ...
    if uploaded_file is not None:
        ...
        try:
            ...
            if work_data:
                ...
                st.json(work_data)    # show what was actually parsed
            else:
                st.warning("⚠️ Parser returned empty result.")
...
def handle_bidder_management():
    ...
    # Step 1: Select specific work (if multiple works)
    selected_work = None
    ...
    # Step 2: Select number of bidders
    ...
    # Step 3: Create input windows for each bidder
    if num_bidders:
        ...
        for i in range(num_bidders):
            ...
            with col1:
                # Option to select from database or add new
            ...
            with col3:
                # Show bid calculation if valid inputs
